...
makeConfig conf =
  ...
  where
    conf' = M.union conf . M.fromList . map (toItem *** toItem) $
      ...
      , "Log"                 ~> "notice stdout" ]
    (~>) = (,)
    f `app` addr = f `ap` parse (B.pack addr) conf'
    testConf = do
      concTests <- parse (B.pack "EnableActiveTesting") conf'
      if concTests
...
instance ConfigValue Bool where
  parse item conf = do
    ...
    case B.map toLower val of
      lc | lc == B.pack "true"  -> return True
         | lc == B.pack "false" -> return False
  ...
  parse item conf = do
    ...
    prependError (cat "Parsing log option " (esc maxLogLen val)
      ...
      severity' <- case B.map toLower severity of
        lc | lc == B.pack "debug"  -> return Debug
           | lc == B.pack "info"   -> return Info
           | lc == B.pack "notice" -> return Notice
           | lc == B.pack "warn"   -> return Warn
           | lc == B.pack "error"  -> return Error
      ...
      target' <- case B.map toLower target of
        lc | lc == B.pack "stdout" -> return ToStdOut
           | lc == B.pack "stderr" -> return ToStdErr
           | lc == B.pack "syslog" -> return ToSysLog
           | lc == B.pack "file" ->
...
parseConfigArgs =
  ...
  where
    ...
    parseArg ["-f",option] = return (B.pack "configfile", B.pack option)
